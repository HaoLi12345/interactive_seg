...
path = '/home/hao/Desktop/3D_train_npz_random_10percent_16G'
path_dest = '/home/hao/Desktop/3D_train_npz_random_10percent_16G'
...
def process(file):
    ...
    #     img_mid = show_mask_cv2((gts[box_dict['z_mid']] == unique_labs[j]).astype(np.uint8), img_mid.astype(np.uint8),
    #                             color=color, alpha=0.5)
        #cv2.imwrite(os.path.join(sanity_dir, os.path.basename(file).replace('.npz', f'_boxIdx{j}.png')), img_mid)

    assert gt.sum() > 0
        # np.savez_compressed(os.path.join(path_dest, os.path.basename(file)), imgs=imgs, gts=gts, boxes=boxes_list,
        #                     spacing=spacing)
    np.savez_compressed(file, imgs=imgs, gts=gts, boxes=boxes_list, spacing=spacing)
